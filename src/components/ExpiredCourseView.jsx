...
import { Archive, BookOpen, CalendarDays, Users } from 'lucide-react'
...
export default function ExpiredCourseView({ courses }) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <p className="text-sm text-gray-400">依開課月份查詢 · 共 {expiredCourses.length} 筆歷史記錄</p>
        {availableMonths.length > 0 && (
          ...
          >
            {availableMonths.map(({ year, month, key }) => (
              <option key={key} value={key}>{year} 年 {month} 月</option>
            ))}
          </select>
        )}
      </div>
          ...
          {filtered.map(c => (
            <div key={c.id} className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
              <div className="flex items-start justify-between mb-3">
                ...
                <span className="text-xs px-2 py-0.5 rounded-full bg-gray-100 text-gray-400 font-medium shrink-0">已結束</span>
              </div>
