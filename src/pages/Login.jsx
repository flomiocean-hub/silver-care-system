...
export default function Login() {
  const { login, completeSuperAdminLogin, loginWithGoogle, googleError, setGoogleError } = useAuth()

  // step: 'auth' → 一般登入頁  |  'pick-org' → 超級管理者選組織
  ...
  async function handleSubmit(e) {
    ...
    setLoading(false)
    if (!result.ok) {
      setError('帳號或密碼錯誤，請再確認')
      return
    }
    if (result.needOrgPick) {
      setStep('pick-org')
    }
    // 非超級管理者：org 已自動帶入，AuthContext 完成登入，App 會跳轉
  }

  function handleOrgConfirm() {
    if (!selectedOrg) return
    completeSuperAdminLogin(selectedOrg)
    // App 偵測到 user + currentOrg 後自動跳轉
  }
      ...
      <div className="min-h-screen bg-surface flex items-center justify-center p-4">
        <div className="w-full max-w-sm bg-white rounded-2xl shadow-sm border border-gray-200 p-6 space-y-5">
          <div className="text-center">
            ...
            <p className="text-xs text-gray-400 mt-1">超級管理者可切換至任何合作單位</p>
          </div>
    ...
    <div className="min-h-screen bg-surface flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 space-y-4">
          ...
          {/* Google 登入（關懷站專員 + 管理者）*/}
          <div>
            <p className="text-xs font-medium text-gray-500 mb-2">關懷站專員 / 管理者</p>
            <button
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs text-gray-400">超級管理者</span>
            <div className="flex-1 border-t border-gray-200" />
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <button
              ...
              className="w-full py-3 bg-gray-800 text-white rounded-xl font-semibold text-sm hover:bg-gray-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {loading ? '登入中…' : '登入'}
            </button>
          </form>
          ...
          <div className="pt-3 border-t border-gray-100">
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex items-center justify-between bg-green-50 rounded-lg px-3 py-2">
                ...
                <span className="text-xs text-green-600">Google 帳號登入（需先被新增）</span>
              </div>
              <div className="flex items-center justify-between bg-blue-50 rounded-lg px-3 py-2">
                <span className="text-xs text-blue-700 font-medium">關懷站專員</span>
                <span className="text-xs text-blue-600">Google 帳號登入（需先被新增）</span>
              </div>
            </div>
          </div>
        </div>
