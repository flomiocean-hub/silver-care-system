...
import { Building2, Phone, MapPin, User, Search, ChevronDown, X, Save, Loader2 } from 'lucide-react'
...
export default function CareStations() {
  ...
  useEffect(() => {
    getCareStations().then(d => { setRecords(d); setLoading(false) })
  }, [])
  ...
  }, [records, district, orgType, status, search])

  const stats = useMemo(() => ({
    total:     records.length,
    prospect:  records.filter(r => r.status === 'prospect').length,
  ...
  async function handleSave() {
    ...
    try {
      ...
      setRecords(prev => prev.map(r =>
        r.id === selected.id
          ? { ...r, status: editStatus, notes: editNotes, assigned_to: editAssigned }
          : r
      ))
      setSelected(s => s ? { ...s, status: editStatus, notes: editNotes, assigned_to: editAssigned } : s)
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="p-4 md:p-6 space-y-4">
      <div>
        <h1 className="text-xl font-semibold text-gray-800">潛在客戶管理</h1>
        <p className="text-sm text-gray-400 mt-1">台北市社區關懷服務據點 · {records.length} 個單位</p>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {(search || district !== '全部' || orgType || status) && (
          <button onClick={() => { setSearch(''); setDistrict('全部'); setOrgType(''); setStatus('') }}
            className="px-3 py-2 text-sm text-gray-500 hover:text-gray-700 border border-gray-200 rounded-lg flex items-center gap-1">
            <X className="w-3.5 h-3.5" /> 清除
          </button>
        )}
        <span className="px-3 py-2 text-sm text-gray-400">共 {filtered.length} 筆</span>
      </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {filtered.length === 0 && (
                  <tr>
                    <td colSpan={6} className="py-16 text-center text-gray-400 text-sm">沒有符合條件的據點</td>
                  </tr>
                )}
              </tbody>
            </table>
        ...
        {selected && (
          <div className="w-full md:w-80 bg-white rounded-xl border border-gray-200 shadow-sm p-5 space-y-4 shrink-0">
            ...
            <div className="space-y-2 text-xs text-gray-500">
              ...
              {selected.villages && (
                <div className="flex items-start gap-2">
                  <span className="text-gray-400">服務村里：</span>
